...
FALLBACK_EMPLOYEE_LIST = "EMPLOYEE_LIST.xlsx" # keep a copy in repo
...
# Custom CSS for card style and header
st.markdown("""
<style>
/* Header */
.header {
...
def load_employee_list():
    if os.path.exists(NAS_EMPLOYEE_LIST_PATH):
        try:
            df = pd.read_excel(NAS_EMPLOYEE_LIST_PATH, dtype=str)
            return df
        except:
            pass
    st.error("❌ Employee list not found!")
    st.stop()
